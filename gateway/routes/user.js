...
router.post('/invoice', function(req, res) {
  ...
  invoice.findOne({'invoiceNumber': requestBody.invoiceNumber, 'sender_address': requestBody.sender_address}).then(function(invoiceDataResponse){
...
router.put('/invoice2', function(req, res) {
  const { id, transaction_hash2, status2 } = req.body;
  invoice.findOne({ '_id': id.toString() }).then(function(invoiceDataResponse) {
    invoiceDataResponse.transaction_hash2 = transaction_hash2;
    invoiceDataResponse.status2 = status2;
    invoiceDataResponse.save({}).then(function(dataSave) {
      res.send({ 'message': 'success', 'data': req.body });
    });
  });
